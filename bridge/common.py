...
class SimulatorBridge(ABC):
    ...
    def run(self, queue, retries=-1):
        bridge_p = Process(
            name="bridge", target=self.bridge_keep_alive, args=(queue, retries)
        )
        bridge_p.start()
        return bridge_p
    ...
    def _run(self, q: Queue):
        ...
        self.simulated_car_thread = threading.Thread(
            target=rk_loop,
            args=(
                functools.partial(self.simulated_car.update, self.simulator_state),
                100,
                self._exit_event,
            ),
        )
        ...
        while self._keep_alive:
            ...
            if not q.empty():
                ...
                if message.type == QueueMessageType.CONTROL_COMMAND:
                    ...
                    if m[0] == "steer":
                        ...
                        self.simulator_state.ignition = (
                            not self.simulator_state.ignition
                        )
                    elif m[0] == "reset":
                        self.world.reset()
                    elif m[0] == "quit":
            ...
            self.simulator_state.is_engaged = self.simulated_car.sm[
                "selfdriveState"
            ].active

            if self.simulator_state.is_engaged:
                throttle_op = clip(
                    self.simulated_car.sm["carControl"].actuators.accel / 1.6, 0.0, 1.0
                )
                brake_op = clip(
                    -self.simulated_car.sm["carControl"].actuators.accel / 3.0, 0.0, 1.0
                )
                steer_op = self.simulated_car.sm[
                    "carControl"
                ].actuators.steeringAngleDeg

                self.past_startup_engaged = True
            elif (
                not self.past_startup_engaged
                and self.simulated_car.sm["selfdriveState"].engageable
            ):
                self.simulator_state.cruise_button = (
                    CruiseButtons.DECEL_SET
                    if self.startup_button_prev
                    else CruiseButtons.MAIN
                )  # force engagement on startup
                self.startup_button_prev = not self.startup_button_prev

            throttle_out = (
                throttle_op if self.simulator_state.is_engaged else throttle_manual
            )
