...
from openpilot.tools.sim.bridge.metadrive.realdrive_process import (
    realdrive_process,
    realdrive_simulation_state,
    realdrive_vehicle_state,
)
from openpilot.tools.sim.lib.common import SimulatorState, World
from openpilot.tools.sim.lib.camerad import W, H


class RealDriveWorld(World):
    def __init__(self, status_q, config, test_duration, test_run, dual_camera=False):
        super().__init__(dual_camera)
        self.status_q = status_q
        self.camera_array = Array(ctypes.c_uint8, W * H * 3)
        self.road_image = np.frombuffer(
            self.camera_array.get_obj(), dtype=np.uint8
        ).reshape((H, W, 3))
        self.wide_camera_array = None
        if dual_camera:
            self.wide_camera_array = Array(ctypes.c_uint8, W * H * 3)
            self.wide_road_image = np.frombuffer(
                self.wide_camera_array.get_obj(), dtype=np.uint8
            ).reshape((H, W, 3))
        ...
        self.realdrive_process = multiprocessing.Process(
            name="realdrive process",
        ...
        self.realdrive_process.start()
        self.status_q.put(QueueMessage(QueueMessageType.START_STATUS, "starting"))

        print("----------------------------------------------------------")
        print("---- Spawning REAL world, this might take a while ----")
        print("----------------------------------------------------------")

        self.vehicle_last_pos = (
            self.vehicle_state_recv.recv().position
    ...
    def read_state(self):
        while self.simulation_state_recv.poll(0):
            ...
            if md_state.done:
                self.status_q.put(
                    QueueMessage(QueueMessageType.TERMINATION_INFO, md_state.done_info)
                )
                self.exit_event.set()

    def read_sensors(self, state: SimulatorState):
        while self.vehicle_state_recv.poll(0):
            md_vehicle: realdrive_vehicle_state = self.vehicle_state_recv.recv()
            ...
            after_engaged_check = (
                is_engaged
                and time.monotonic() - self.first_engage >= 5
                and self.test_run
            )
            ...
            dist_threshold = 1
            if (
                x_dist >= dist_threshold or y_dist >= dist_threshold
            ):  # position not the same during staying still, > threshold is considered moving
            ...
            #         self.status_q.put(
            #             QueueMessage(
            #                 QueueMessageType.TERMINATION_INFO,
